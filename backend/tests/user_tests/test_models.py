...
class TeacherSecretKeyModelTest(TestCase):
    ...
    def test_teacher_secret_key_creation(self):
...
class SchoolClassModelTest(TestCase):
    ...
    def test_school_class_unique_slug(self):
    ...
    def test_school_class_save_method(self):
        school_class = SchoolClass(title="1B", slug="1b")
        school_class.save()
        self.assertEqual(school_class.title, "1B")
        self.assertEqual(school_class.slug, "1b")

    def test_school_class_save_method_with_unique_slug(self):
        school_class = SchoolClass(title="1C", slug="1a")
        with self.assertRaises(IntegrityError):
            school_class.save()

    def test_school_class_save_method_with_existing_title_and_slug(self):
